                ...
                <div className='flex items-center'>
                    <div className='grow'>
                        ...
                        <input
                            ...
                            className="bg-white-800  h-11 border border-spacing-2  dark:border-primary product_input_filed border-black text-sm rounded-lg rounded-r-none border-r-0 focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-black-800 dark:border-black-600 dark: dark:text-white-900 dark:focus:ring-sky-600 dark:focus:border-sky-600"
                            aria-invalid={errors[rest.name] ? "true" : "false"}
                        />
                    </div>
                    <div className='rounded-lg w-9 rounded-l-none border border-l-0 h-11 bg-white-800 dark:bg-black-700  dark:border-primary ' style={{ marginTop: "27px" }}>
                        {
                            !show ? <AiFillEye onClick={handleShow} className='mt-3 ml-2 dark:text-primary ' style={{ cursor: 'pointer' }} /> :
                                <AiFillEyeInvisible onClick={handleShow} className='mt-3 ml-2 dark:text-primary' style={{ cursor: 'pointer' }} />
                        }
                    </div>
                </div>
        ...
        default:
            return <div className='mt-2 '>
                ...
                <input
                    {...register(rest.name, { required: true })}
                    aria-invalid={errors[rest.name] ? "true" : "false"}
                    {...rest} type={type} className="product_input_filed border-spacing-2  dark:border-primary  bg-white-800 border border-black- text-sm rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-black-800 dark:border-black-600 dark: dark:text-white-900 dark:focus:ring-sky-600 dark:focus:border-sky-600" />
                {errors[rest.name] && <span className='text-error'>{rest.name} is required</span>}
            </div>;
    }
